...
public class GLFW
{
    ...
    static {
        ((Buffer) joystickData).limit(0);
        ((Buffer) buttonData).limit(0);
    }
    ...
    private static ArrayMap<Long, Integer> mGLFWCursorMap;
    private static long mGLFWCursorHandle = 0x10000L;
    private static int mLastSentCursorType = Integer.MIN_VALUE;
    ...
    */
    ...
    public static long glfwCreateCursor(@NativeType("const GLFWimage *") GLFWImage image, int xhot, int yhot) {
        long cursorHandle = ++mGLFWCursorHandle;
        mGLFWCursorMap.put(cursorHandle, GLFW_ARROW_CURSOR);
        return cursorHandle;
    }
    public static long glfwCreateStandardCursor(int shape) {
        long cursorHandle = ++mGLFWCursorHandle;
        mGLFWCursorMap.put(cursorHandle, shape);
        return cursorHandle;
    }
    public static void glfwDestroyCursor(@NativeType("GLFWcursor *") long cursor) {
        mGLFWCursorMap.remove(cursor);
    }
    public static void glfwSetCursor(@NativeType("GLFWwindow *") long window, @NativeType("GLFWcursor *") long cursor) {
        Integer cursorType = mGLFWCursorMap.get(cursor);
        int resolvedCursorType = cursorType == null ? GLFW_ARROW_CURSOR : cursorType;
        if (resolvedCursorType != mLastSentCursorType) {
            mLastSentCursorType = resolvedCursorType;
            CallbackBridge.nativeSetCursorType(resolvedCursorType);
        }
    }
