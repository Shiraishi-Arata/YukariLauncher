...
import com.arata.yukarilauncher.databinding.FragmentCustomMouseBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        openDocumentLauncher = registerForActivityResult<Array<String>, Uri>(ActivityResultContracts.OpenDocument()) { result: Uri? ->
            result?.let { uri ->
                ...
                Task.runTask {
                    val importedFile = FileTools.copyFileInBackground(requireActivity(), uri, mousePath().absolutePath)
                    if (CursorPackUtils.isCursorArchive(importedFile)) {
                        val extractedDir = CursorPackUtils.extractCursorArchive(importedFile, mousePath())
                        importedFile.delete()
                        val selectedName = extractedDir.name
                        if (YLTools.isSupportedMouseSource(extractedDir)) {
                            AllSettings.customMouse.put(selectedName).save()
                        }
                    }
