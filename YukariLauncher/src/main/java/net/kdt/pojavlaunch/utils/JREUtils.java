...
import static com.arata.yukarilauncher.utils.path.PathManager.DIR_MOD_LIBRARY;
 ...
 */
    private static void setJavaEnv(Map<String, String> envMap, String jreHome) {
        ...
        envMap.put("YUKARI_MOD_LIBRARY_DIR", DIR_MOD_LIBRARY);
 ...
 */
    private static void setRendererEnv(Map<String, String> envMap) {
        ...
        if (rendererId.startsWith("opengles")) {
            ...
            envMap.put("LIBGL_NOINTOVLHACK", "1");
            envMap.put("LIBGL_NOERROR", "1");
        } else if (rendererId.equals("nggl4es")) {
            envMap.put("LIBGL_USE_MC_COLOR", "1");
            envMap.put("DLOPEN", "libspirv-cross.so");
            ...
            envMap.put("MESA_GL_VERSION_OVERRIDE", rendererId.equals("gallium_virgl") ? "4.3" : "4.6");
            envMap.put("MESA_GLSL_VERSION_OVERRIDE", rendererId.equals("gallium_virgl") ? "430" : "460");
            envMap.put("force_glsl_extensions_warn", "true");
            envMap.put("allow_higher_compat_version", "true");
            envMap.put("allow_glsl_extension_directive_midshader", "true");
            envMap.put("MESA_LOADER_DRIVER_OVERRIDE", "zink");
            envMap.put("LIB_MESA_NAME", loadGraphicsLibrary());
            if (rendererId.equals("gallium_virgl")) {
                envMap.put("POJAV_RENDERER", "gallium_virgl");
                envMap.put("OSMESA_NO_FLUSH_FRONTBUFFER", "1");
            } else if (rendererId.equals("vulkan_zink")) {
                envMap.put("POJAV_RENDERER", "vulkan_zink");
            } else if (rendererId.equals("freedreno")) {
 ...
 */
    private static void setEnv(String jreHome, final Runtime runtime, Version gameVersion) throws Throwable {
        ...
        if (gameVersion != null) {
            ...
            envMap.put("YUKARI_VERSION_CODE", String.valueOf(YLTools.getVersionCode()));
        }
 ...
 */
    private static void launchJavaVM(
        ...
        File sableRapierLib = new File(DIR_MOD_LIBRARY, "libsable_rapier-" + arch + ".so");
        if (sableRapierLib.exists()) {
            userArgs.add("-Dsable_rapier_path=" + sableRapierLib.getAbsolutePath());
        }
        
        File imguiMoulberry = new File(DIR_MOD_LIBRARY, "libimgui-moulberry92-java-" + arch + ".so");
        if (imguiMoulberry.exists()) {
            userArgs.add("-Dimgui_moulberry_path=" + imguiMoulberry.getAbsolutePath());
        }
